...
class TUIConversationViewModel extends ChangeNotifier {
  ...
  _onConversationListChanged(List<V2TimConversation> list) {
    for (int element = 0; element < list.length; element++) {
      var message = list[element].lastMessage;
      if (message?.elemType == MessageElemType.V2TIM_ELEM_TYPE_GROUP_TIPS &&
          (message?.groupTipsElem?.groupChangeInfoList?.length ?? 0) > 0) {
        List<V2TimGroupChangeInfo?> arr = message?.groupTipsElem?.groupChangeInfoList?.where((element) => element?.type == GroupChangeInfoType.V2TIM_GROUP_INFO_CHANGE_TYPE_FACE_URL).toList() ?? [];
        if (arr.length > 0) {
          list[element].faceUrl = arr.first?.value;
          WBManager().groupAvatarDic[message?.groupID ?? ""] = arr.first?.value ?? "";
        }

      }
      if (WBManager().groupAvatarDic.keys.contains(message?.groupID)) {
        list[element].faceUrl = WBManager().groupAvatarDic[message?.groupID ?? ""];
      }
